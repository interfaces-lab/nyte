...
```bash
pnpx react-doctor@latest rules explain react-doctor/no-array-index-as-key
```
...
```bash
pnpx react-doctor@latest rules list                         # every rule + its effective severity
pnpx react-doctor@latest rules list --configured            # only what your config changed
pnpx react-doctor@latest rules list --category Performance   # filter by category
pnpx react-doctor@latest rules explain <rule>               # why it matters + how to configure
pnpx react-doctor@latest rules disable <rule>               # rule never runs
pnpx react-doctor@latest rules enable <rule>                # turn back on at its recommended severity
pnpx react-doctor@latest rules set <rule> warn              # off | warn | error
pnpx react-doctor@latest rules category "React Native" off   # whole category
pnpx react-doctor@latest rules ignore-tag design            # skip a rule family (design, test-noise, …)
pnpx react-doctor@latest rules unignore-tag design
```
