...
3. Pick the runners. Use the `arena runners` list from `~/.agents/model-roles.md` when present; otherwise use the session model. Prefer distinct model families or effort tiers when configured; otherwise vary the prompt/lens instead. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
