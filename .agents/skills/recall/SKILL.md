...
Transcripts live at `~/.claude/projects/<slug>/<uuid>.jsonl`, where `<slug>` is the absolute workspace path with each "/" turned into "-", leading dash included (so `/Users/you/proj` becomes `-Users-you-proj`). Every line is one chat message.
