...
- `model`: your configured how-explorer model from `~/.agents/model-roles.md` (otherwise the session model)
- `readonly`: `true`
...
- `subagent_type`: `general-purpose`
- `model`: your configured how-explainer model from `~/.agents/model-roles.md` (otherwise the session model)
- `readonly`: `true`
...
After the explanation is complete, spawn one architectural critic per model in your configured how-critics list from `~/.agents/model-roles.md` (otherwise use the session model; prefer distinct model families or effort tiers when configured, otherwise vary the prompt/lens instead), all in a single message.
