...
Before spawning investigators, list the connected MCP servers and their tools (ToolSearch / the tool listing).
...
- `model`: your configured why-investigators model from `~/.agents/model-roles.md` (otherwise the session model)
...
- `subagent_type`: `general-purpose`
- `model`: your configured why-synthesizer model from `~/.agents/model-roles.md` (otherwise the session model)
