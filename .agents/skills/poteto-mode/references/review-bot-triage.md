...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles review-bot or other review-automation comments (Bugbot and the like). The goal is not to ignore the review bot by default. The goal is to stop treating every comment as a required code change.
