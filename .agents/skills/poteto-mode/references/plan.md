...
- Pass `model:` per the roles in `~/.agents/model-roles.md` when present; a role with no entry runs on the session model (omit `model`).
...
If a phase creates or edits a skill, the phase instructs the implementer to follow the **authoring-a-skill** playbook (`../playbooks/authoring-a-skill.md`).
...
**Runtime.** Exercise the feature on the matching surface via the project's verify skill:

- Browser / Electron / Web / CLI / TUI: the project's verify skill (generate one with `/create-verification-skill`).
