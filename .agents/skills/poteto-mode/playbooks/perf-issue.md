...
1. Capture a baseline trace via the project's verify skill (generate one with `/create-verification-skill`).
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue model (the `perf-issue` role from `~/.agents/model-roles.md` when present; otherwise the session model); review the diff. Capture a post-fix trace.
