...
4. Verify each component against its baseline via image diff on the matching surface via the project's verify skill (generate one with `/create-verification-skill`). A nonzero diff is a fail; investigate the pixel delta, don't wave it through. `/loop` per component until the diff is zero.
