...
5. Verify on the matching surface. For a visual decision, screenshot each variant via the project's verify skill (generate one with `/create-verification-skill`) and drive the interaction; the eye is the test. For a behavioral or timing decision, observe the thing you are deciding by logging the timing, printing the output, or watching the render. The observation is the test here, not an assertion.
