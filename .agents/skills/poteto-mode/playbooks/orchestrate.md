...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the task's status in the task list (`/tasks`). Transcript mtime is not liveness.
