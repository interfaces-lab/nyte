...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured bug-fix model (the `bug-fix` role from `~/.agents/model-roles.md` when present; otherwise the session model) with a specific scope; review the diff.
