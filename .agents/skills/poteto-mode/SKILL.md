...
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **writing-guidelines** skill (`/writing-guidelines`).
- Before commit → the **unslop** skill applied to the diff.
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → the project's verify skill (generate one with `/create-verification-skill`). For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
...
- A review bot (Bugbot or other PR review automation) commented → skeptical posture. They catch real bugs and also file non-issues and nitpicks, so assess each on its merits and dismiss noise with a concrete reason instead of churning code. Triage fix / dismiss / ask per `references/review-bot-triage.md`.
...
**Defaults for every `Agent` call.** Background where the harness supports it, read-only agent types (e.g. `Explore`) for read-only roles, file pointers not inlined context, model per role from `~/.agents/model-roles.md` when present. Code delegates tier by difficulty. The hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your strongest judgment role when the task needs judgment or the intent is vague, and to your strongest instruction-following role when the work is a precisely specified sequence of steps to execute to the letter; trivial mechanical edits go to your fast code role. Per-role lines in `~/.agents/model-roles.md` override these defaults and the model choices in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`); a role with no line runs on the session model (omit the Agent `model` param).
