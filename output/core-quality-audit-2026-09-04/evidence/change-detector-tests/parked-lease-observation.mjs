...
import { definePlugin, inlinePlugin } from '/Users/workgyver/Developer/nyte/packages/core/src/plugins/types.ts';
...
const model = {id: 'test', name: 'Test', api: 'openai-responses', provider: 'openai', baseUrl: 'https://example.invalid', reasoning: false, input: ['text'], cost: usage.cost, contextWindow: 10000, maxTokens: 100};
let providerRequests = 0;
let toolExecutions = 0;
const plugin = inlinePlugin(definePlugin({id: 'ask', session(api) {
  api.tools.add(draft => draft.set('ask', {
    name: 'ask', description: 'Ask a question', parameters: {type: 'object', properties: {}},
    execute: async () => { toolExecutions++; throw new ToolWait(); },
    wake: async () => ({kind: 'settle', result: {content: [{type: 'text', text: 'blue'}], details: {}}}),
  }));
}}));
const nyte = await createNyte({store, model, models: {getModels: () => [model], getModel: () => model}, plugins: [plugin], env: {cwd: '/tmp'}, streamFn: () => {
  providerRequests++;
  const stream = createAssistantMessageEventStream();
  const answer = {role: 'assistant', content: [{type: 'toolCall', id: 'ask-1', name: 'ask', arguments: {}}], api: model.api, provider: model.provider, model: model.id, usage, stopReason: 'toolUse', timestamp: 1};
...
try {
  ...
  await Promise.race([acquiredWhileParked.promise, new Promise((_, reject) => {timer = setTimeout(() => reject(new Error('No delayed wake within 2 seconds')), 2000);})]);
  clearTimeout(timer);
  ...
  console.log(JSON.stringify({wait: parked.kind, laterPhase: current?.phase.kind, laterLeasePresent: current?.lease !== undefined, settledLeasePresent: settled?.lease !== undefined, before, after}));
} finally {
  clearTimeout(timer);
  parkingEvent.resolve();
  finishAcquire.resolve();
