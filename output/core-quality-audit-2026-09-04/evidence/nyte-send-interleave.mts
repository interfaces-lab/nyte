...
import type {
  Store,
  Session,
} from "/Users/workgyver/Developer/nyte/packages/core/src/kernel/store.ts";
...
import {
  pending,
  pendingIn,
} from "/Users/workgyver/Developer/nyte/packages/core/src/kernel/queue.ts";
...
const [first, retry] = await Promise.all([
  nyte.messages.send({ sessionId: id, content: "prompt-a", agent: "agent-a", key: "key-a" }),
  nyte.messages.send({ sessionId: id, content: "prompt-b", agent: "agent-b", key: "key-b" }),
]);
...
console.log(
  JSON.stringify({
    first,
    retry,
    queued: (await pendingIn(inspect, { head: "main", lane: "steer" })).map(
      (item) => item.change.body,
    ),
  }),
);
await step(
  ...
  { head: "main", landing: DEFAULT_LANDING },
);
const runOid = await inspect.refs.read(runRef("main"));
console.log(
  JSON.stringify({
    landed: (await branch(inspect.objects, await inspect.refs.read(headRef("main")))).map(
      (item) => item.commit.body,
    ),
  }),
);
console.log(
  JSON.stringify({ firstRun: runOid === null ? null : await inspect.objects.get(runOid) }),
);
await inspect.close();
