...
import { pending } from '/Users/workgyver/Developer/nyte/packages/core/src/kernel/queue.ts';
...
const first = await nyte.messages.send({ sessionId: id, content: 'first prompt', agent: 'agent-a', key: 'same-key' });
const retry = await nyte.messages.send({ sessionId: id, content: 'ignored retry prompt', agent: 'agent-b', key: 'same-key' });
const inspect = await backend.open(id);
console.log(JSON.stringify({ first, retry, queued: (await pending(inspect, 'main')).map(item => item.change.body) }));
await inspect.close();
