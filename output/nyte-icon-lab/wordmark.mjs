...
export function wordmark({color='#d2ddff',weight=13,spacing=4}={}){
 let x=0;
 const pieces=letters.map((d,i)=>{const path=`<path d="${d}" transform="translate(${x} 0)"/>`;x+=[76,76,60,72][i]+spacing;return path;});
 return {width:x-spacing+4,svg:`<g fill="none" stroke="${color}" stroke-width="${weight}" stroke-linecap="round" stroke-linejoin="round">${pieces.join('')}</g>`};
}
export function renderLockup(settings,layout='side',options={},id='lockup'){
 ...
 const w=640,h=layout==='stack'?330:220;
 let art='';
 if(layout==='side'){
  const size=132,scale=.87,total=size+gap+mark.width*scale,x=(w-total)/2;
  ...
  art=`<g transform="translate(261 24)">${renderIcon(settings,id+'-icon',118)}</g><g transform="translate(${(w-mark.width*.8)/2} ${148+gap*.45}) scale(.8)">${mark.svg}</g>`;
 }else{
  const scale=.85,total=mark.width*scale,x=(w-total)/2;
  const left=x-gap-45,right=x+total+gap+45;
