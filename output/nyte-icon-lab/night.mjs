export const marks = [
  { name:'Open interval', slug:'open-interval', note:'Two soft planes. One clear opening.', description:'A curved opening gives the mark its identity. The light sits between two surfaces, with enough contrast to hold at Dock size.' },
  { name:'Soft current', slug:'soft-current', note:'A continuous curve, moving upward.', description:'A rising ribbon carries the eye through the mark. The turn is broad and soft, with a darker underside to give it depth.' },
  { name:'First thought', slug:'first-thought', note:'A small glow above a quiet horizon.', description:'Light gathers at one edge of a dark field. This is the quietest direction, with the fewest shapes and the most open space.' },
];
export function renderMark(index=0,light=64,id='nyte',size=512){
 ...
 const shapes=[
  `<path d="M111 297 C128 203 209 141 294 153 C350 160 386 194 405 239 C342 212 296 224 254 271 C210 321 156 333 111 297Z" fill="url(#${id}-light)"/>
   ...
   <path d="M118 311 C176 347 224 315 262 274 C306 231 352 226 400 250" stroke="#c9d7ff" stroke-opacity="${.12+glow*.32}" stroke-width="2" fill="none"/>`,
  `<path d="M123 353 C92 322 113 275 160 242 L300 142 C331 119 370 126 390 152 C412 182 397 210 367 234 L223 338 C188 365 155 380 123 353Z" fill="url(#${id}-lower)"/>
   ...
   <path d="M127 349 C160 350 188 321 213 294 L299 196" fill="none" stroke="#ecf0ff" stroke-width="1.5" stroke-opacity="${.15+glow*.3}"/>`,
  `<ellipse cx="266" cy="252" rx="140" ry="96" fill="url(#${id}-bloom)"/>
   ...
   <path d="M112 325 C170 300 217 264 267 262 C320 260 357 282 404 304" fill="none" stroke="#d2ddff" stroke-opacity="${.2+glow*.42}" stroke-width="2"/>`
 ][index] || '';
 ...
 <radialGradient id="${id}-bloom"><stop stop-color="#b6c8fa" stop-opacity="${.14+glow*.48}"/><stop offset="1" stop-color="#7898eb" stop-opacity="0"/></radialGradient>
 ...
 <g clip-path="url(#${id}-clip)"><ellipse cx="255" cy="220" rx="195" ry="166" fill="url(#${id}-bloom)"/><g filter="url(#${id}-depth)" opacity="${.78+glow*.22}">${shapes}</g><rect x="50.75" y="50.75" width="410.5" height="410.5" rx="90" stroke="#d2ddff" stroke-opacity=".1" stroke-width="1.5" fill="none"/></g></svg>`;
}
