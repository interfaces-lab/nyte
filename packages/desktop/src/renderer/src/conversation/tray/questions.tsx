      ...
      {(selection.multiple === true || selection.other !== undefined) && (
        <form
          ...
          onSubmit={(event) => {
            ...
            send(custom === "" ? { choices: selected } : { choices: selected, other: custom });
          }}
        >
...
/** Questions wait above the composer, where the answer is typed, rather than in the transcript. */
      ...
      <Tray open={count > 0 || children.isError} label="Questions">
        <div {...props(trayStyles.header)}>
          <span {...props(trayStyles.title)}>
            {count === 1 ? "Question" : `${String(count)} questions`}
          </span>
        </div>
        <div
