...
import { sessionId, treeId } from "@nyte-ai/protocol";
...
test("a provisional live diff is refreshed once after the run becomes terminal", () => {
  ...
  assert.equal(freshness.needsRefresh(session, run, false, tree), true);
  assert.equal(freshness.needsRefresh(session, run, true, tree), true);

  freshness.recordCheck(session, run, true);

  assert.equal(freshness.needsRefresh(session, run, true, tree), false);
});
