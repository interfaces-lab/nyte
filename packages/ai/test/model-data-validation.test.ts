...
function createFixture() {
  const packageRoot = mkdtempSync(join(tmpdir(), "nyte-model-data-"));
  temporaryRoots.push(packageRoot);
  const providersDir = join(packageRoot, "src", "providers");
  const dataDir = join(providersDir, "data");
  mkdirSync(dataDir, { recursive: true });
  writeFileSync(join(providersDir, "test-provider.models.ts"), "");
  ...
  const manifest = writeFixtureData(dataDir, structure, values);
  return { dataDir, packageRoot, structure, values, manifest };
}
...
describe("generated model data validation", () => {
  it("rejects a missing upstream model from an exact generated allowlist", () => {
    expect(() =>
      assertExactModelIds("qwen-token-plan-individual", ["model-a", "model-b"], ["model-a"]),
    ).toThrow("qwen-token-plan-individual model IDs do not match (missing: model-b)");
  });
  ...
  it("reads and validates API-grouped model data", () => {
    ...
    expect(readModelDataStructure(result.packageRoot, ["test-provider"])).toEqual(result.structure);
    expect(() => validateModelDataDirectory(result.structure, result.dataDir)).not.toThrow();
  });

  it("rejects a missing model data directory", () => {
    const result = createFixture();
    rmSync(result.dataDir, { recursive: true });
    expect(() => validateModelDataDirectory(result.structure, result.dataDir)).toThrow(
      "does not exist",
    );
  });
  ...
  it.each([
    ...
    fixture.values["model-a"][field] = value;
  ...
  it("rejects incompatible schema and generation stamps", () => {
    ...
    const manifest = { ...fixture.manifest, structureHash: "stale" };
  ...
  it("rejects an invalid generation timestamp", () => {
    const fixture = createFixture();
    const manifestPath = join(fixture.dataDir, MODEL_DATA_MANIFEST_FILE);
    const manifest = { ...fixture.manifest, generatedAt: "invalid" };
    writeFileSync(manifestPath, `${JSON.stringify(manifest)}\n`);
    expect(() => validateModelDataDirectory(fixture.structure, fixture.dataDir)).toThrow(
  ...
  it("rejects a configured provider without a generated shard", () => {
    const result = createFixture();
    expect(() => readModelDataStructure(result.packageRoot, ["test-provider", "missing"])).toThrow(
