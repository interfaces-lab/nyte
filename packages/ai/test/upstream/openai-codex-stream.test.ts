...
function decodeCodexRequestBody(
  ...
  if (body instanceof Uint8Array) {
    return JSON.parse(Buffer.from(zstdDecompressSync(body)).toString("utf8")) as Record<
      string,
      unknown
    >;
  }
...
describe("openai-codex streaming", () => {
  ...
  it("sets Codex strict mode explicitly and honors constrained sampling", async () => {
    ...
    const sse = buildSSEPayload({ status: "completed" });
    let requestedTools:
      | Array<{ type?: string; name?: string; strict?: boolean | null }>
      | undefined;
  ...
  it.each(["gpt-5.3-codex", "gpt-5.4", "gpt-5.5"])(
  ...
  it.each([
    [
      "retry-after-ms",
      () => ({ "content-type": "application/json", "retry-after-ms": "1500" }),
      1500,
    ],
    [
      "retry-after seconds",
  ...
  it("zstd-compresses SSE request bodies", async () => {
    ...
    const decoded = JSON.parse(
      Buffer.from(zstdDecompressSync(capturedBody as Uint8Array)).toString("utf8"),
    ) as {
