/** GitHub Copilot's hosted model catalog, filtered by the signed-in account's model IDs. */
...
export function githubCopilotProvider(options: GitHubCopilotOAuthOptions = {}) {
  let getKnownModelIds: () => ReadonlySet<string> = () => new Set();

  const provider = createProvider<"anthropic-messages" | "openai-completions" | "openai-responses">(
    {
      id: "github-copilot",
      name: "GitHub Copilot",
      ...
      auth: {
        ...
        oauth: githubCopilotOAuth(options, () => getKnownModelIds()),
      },
  ...
  getKnownModelIds = () => new Set(provider.getModels().map((model) => model.id));
