...
```
platform-tokens.stylex.ts    colorVars["--nyte-color-primary"]
...
- A primitive **must** import `colorVars` (and the other `*Vars` groups) from
  `@nyte-ai/ui/platform-tokens.stylex`. StyleX only keeps `--nyte-*` names through a
  direct `defineVars` import. Reading through `tokens.color` hashes them. Literal colors in
