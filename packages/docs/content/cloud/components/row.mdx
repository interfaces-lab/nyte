...
- An icon-only button inside `Row.Actions` needs `aria-label`. `revealActions` hides the lane with
  opacity, so it stays in the tab order, and focus anywhere in the row brings it back.
