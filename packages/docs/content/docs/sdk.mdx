...
| Entry                 | Imports                                                                                                                                                                                                                                |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@nyte-ai/core`       | `createNyte`, the wire data types, `MAIN`, `DEFAULT_LANDING`, and what a host needs to compose. The operation table, envelopes, SSE codec, and `CursorExpired` stay in `@nyte-ai/protocol`; the projections live in `@nyte-ai/client`. |
...
A parked participant selection needs no live process. Its refs hold the choices, optional own-answer
...
A parent agent can call `wait_task({ jobId })` to join an existing background subagent and
continue the same run when its report is ready. This is a tool, not an SDK `jobs.wait` method.
It observes the job without re-running or taking ownership of it. Cancelling only the wait
does not cancel the observed task; aborting a run still cancels that run's own jobs.
...
only that job. `runs.abort` cancels foreground and background work owned by that run, but leaves
independent user jobs running. These controls work while the parent run is waiting; do not gate
...
without rerunning it. A terminal background job submits its state and output as an idempotent
completion on the originating head, through core's private `background` lane. It can join an
active run at a response boundary, but cannot start model work on its own. Without an active run,
it waits for explicit user input. Completions are separate from editable user messages. Clients
do not need to add that lane to their landing policy or send completion messages themselves.
...
The checkpoint a compaction writes still arrives as an ordinary `commit`. The `compaction` event is
...
2. Gate project plugins on workspace trust (`WorkspaceStore` from `@nyte-ai/host`). The grant lives in
   `~/.nyte/workspaces.json` and is inherited from a trusted ancestor. Print mode will not grant trust.
3. Resolve the plugin list with `resolvePlugins`: built-ins, then `~/.nyte/plugins`, then
   `<cwd>/.nyte/plugins`, filtered by the merged `nyte.json` manifests (`~/.nyte` and
   `<cwd>/.nyte`), which also name MCP servers. Watch those sources with
   `watchPluginDirectories` and pass `nyte.holdPlugins` as its `hold`, so a runner's next step
   waits for `setPlugins` and the model sees a plugin it just wrote.
