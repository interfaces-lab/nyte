...
Completions use core's private `background` lane, and a child's answered request lands on its parent
the same way. User input starts model work. A delegate's answer can also start one continuation when
...
active. Clients do not add that lane to their landing policy or send completion messages
themselves.
