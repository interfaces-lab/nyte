...
Plugin authoring is `@nyte-ai/core/plugins`; `@nyte-ai/plugin` re-exports it verbatim. `SqliteStore`
...
refused with a `TypeError`, since no runner would land it. Enter in the TUI steers; `ctrl+enter` queues.
`messages.redeliver` moves a still-pending item between lanes; `messages.cancel` withdraws one. A cancel
