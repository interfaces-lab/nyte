...
its end and output as an idempotent completion on the originating head, through core's private
`background` lane; a child's answered request lands on its parent the same way. A completion can
join an active run at a response boundary, but cannot start model work on its own. Without an
active run, it waits for user input. Clients do not add that lane to their landing policy or send
...
6. Supply `workspace.vcs` (`createGitVcs` from `@nyte-ai/host`) so `nyte.workspace.vcs` answers
   ...
   session's directory. Without it, reads answer empty and writes answer `failed`.
