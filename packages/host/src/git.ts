...
function fileRevisionPart(cwd: string, path: string): string {
  const metadata = entryAt(resolve(cwd, path));

  if (metadata === undefined) return `${path}:missing`;
...
async function readSide(cwd: string, path: string, side: Side): Promise<Buffer | null> {
  if (side.kind === "worktree") {
    ...
    return entryAt(absolute)?.isFile() ? readFile(absolute) : null;
  }
