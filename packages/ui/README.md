...
Those raw colors are not a second set of values. `platform-tokens.stylex.ts` declares each appearance pair once in `lightPalette` and `darkPalette`, two `stylex.defineConsts` groups, and builds every `light-dark()` color token from them. A build that compiles StyleX, which React Strict DOM does on native, inlines the pairs as literals, so `platform-colors.ts` exists only for the native build that cannot yet compile the module.

After editing `platform-tokens.stylex.ts`, run `pnpm --dir packages/ui sync:tokens` to regenerate both `platform-tokens.css` and `platform-colors.ts`. The generator imports the module with `defineVars` and `defineConsts` stubbed to the identity function, so it reads the declared values rather than parsing them, and it rejects a color token whose value does not come from both palettes, because that color would be missing on native.
