...
- Follow the [source layout](README.md#source-layout). Keep feature code in its owning flat folder; do not add generic `services`, `hooks`, or `utils` directories or barrel exports. `src/app/` holds Expo Router files only: keep `src/app/_layout.tsx` limited to startup, providers, and the connection gate, and keep route files thin (parse params, render the owning feature's screen).
